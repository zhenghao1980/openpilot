...
(B8PA port: adapted from sunnypilot onroad/developer_ui/__init__.py —
 element imports redirected to sp_developer_ui_elements, torque-override
 branch removed to match stock ui_state)
"""
from enum import IntEnum

import pyray as rl
from openpilot.selfdrive.ui.ui_state import ui_state
from openpilot.selfdrive.ui.onroad.sp_developer_ui_elements import (
...
class DeveloperUiRenderer(Widget):
  ...
  def _update_state(self) -> None:
    self.dev_ui_mode = DeveloperUiState(ui_state.developer_ui)
  ...
  def _draw_bottom_dev_ui(self, rect: rl.Rectangle) -> None:
    ...
    if sm['controlsState'].lateralControlState.which() == 'torqueState':
      if sm.valid['lateralTorqueParameters']:
        elements.extend([
