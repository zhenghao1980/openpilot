...
裁定），**修改模式统一沿用社区分支惯例及所有者当前奥迪分支的个性化 UI 改造方式**
（同风格新增，不另立规范）；以下为基线设计，呈现形式可自由发挥；唯一正确性约束：
真 Chestnut 插入时的官方显示行为不得被破坏。
...
- 降帧率（10/5Hz）时即**发送降频**：客户端每帧检查"距上次发送 ≥ 1/r 秒则发送当前
  最新帧快照"，每次请求携带的都是最新输入——与官方 frame_skip=4 时序采样语义严格
  同构，两种模式均合法；排队变深时客户端丢弃最旧未发帧，与服务端背压同构。
...
- 单 UDP socket，六类消息：`BEACON`（服务发现信标，见 4.3）、`HELLO`/`META`
  （握手与能力协商，META 携带 **big 模型文件 sha256**，客户端比对不一致即拒会话，
  防止两端版本漂移导致 output_slices 悄悄错乱）、`INFER`（上行推理请求）、
...
5. **UI 属可个性化改造面**（所有者裁定）：社区分支本大量改造 UI，非核心安全代码，
   允许自由设计（含 §3.5 徽标可做 richer 呈现：动效、融合面板等）；**修改模式统一
   沿用社区分支惯例与所有者当前分支（奥迪 B8）已有的个性化 UI 改造方式**——同风格
   直接加代码，不另立规范；唯一保留的正确性约束：真 Chestnut 插入时的官方显示行为
   不得被破坏；
...
## 9. 实现待定项收口（2026-09-09 所有者审定，v2 实现前定案）

架构级三决策（所有者批准推荐方案）：

1. **UI 数据通道走 Params**（§3.5 定案）：`RemoteModelPresent` / `RemoteModelStats`
   用 Params JSON、UI 轮询；不动 cereal 定义，守住"不改任何消息契约"红线。
   1Hz 小 JSON 写入开销实测可忽略；
2. **融合发生在解析后的语义空间**（定案）：`FusionModelState` 内部实例化**两个官方
   Parser**（小模型一个；远端 raw 向量返回后再跑一个），plan 拼接 / lane 按距离分工 /
   lead 平移全部在 plan/lanes/lead 结构化数据上进行，融合结果直接填 modelV2——
   `fill_model_msg` 完全不动；
3. **META 模型版本握手**（§4.2 定案）：META 携带 big 模型文件 sha256，客户端比对
   不一致即拒会话（视同链路不可用，落小模型）。

实现层默认约定（无异议，直接执行）：

4. **心跳超时→降级单一路径**：I/O 线程发现心跳丢失 → 写 `ChestnutActive=False` →
   下一次 `run()` 主动抛 `RemoteModelError` 走现成 stock 降级分支；降级永远只有这
   一条路径，杜绝"I/O 线程写参数"与"主循环异常"双路径竞争；
5. **RESP 输出校验**：远端回传 raw 向量复刻官方 `modeld.py:219` 非有限值校验，
   不通过则丢弃该锚点，不进入融合；
6. **客户端锚点闸门**：每帧检查"距上次发送 ≥ 1/r 秒则发当前最新帧"；发送队列积压
   时丢最旧未发帧（与服务端背压同构）；
7. **平移对齐基准**：锚点时刻 = `client_send_ts + RTT/2`，用 RESP 携带的服务端
   时间戳校准两端时钟偏移（单次握手估算、心跳持续修正）；staleness = 当前时刻 −
   锚点时刻；
8. **w 冷启动斜坡**：会话建立与重连恢复后，w 从 0 线性斜坡升至目标值（前 2 秒），
   避免握手完成瞬间全量信任远端；
9. **w 初版函数**：`w = exp(−staleness/τ₀)`（τ₀≈150ms 初值），过渡带按 plan_stds
   归一化加权——**标注为初版形式，进入验证走廊标定后迭代**，不视为定稿；
10. **标定参数清单**：lane 近场距离阈值（暂定 20m）、plan 过渡带宽度（暂定
    0.3-0.5s）统一列为标定参数，由 §6 验证走廊实测定值，代码中集中一处可配；
11. **分片大小**：默认 32KB、可配置；验收时对比 1400B / 8KB / 32KB 三档实测
    （千兆直连丢包极低，允许 IP 分片换取 syscall 减少，整帧 CRC 兜底）；
12. **会话唯一性与重连**：READY 态接受首个 HELLO，会话中其他 HELLO 回 BUSY 拒绝；
    客户端链路死后退避重连（1s→2s→…→30s 封顶），与服务端恢复广播配合；
13. **CTRL 载荷**（§4.2 定案）：编码切换指令、统计快照请求；
14. **安全声明**：网段内无认证——两端均为车主自有设备，信任边界 = 物理持有
    USB 网线，不防网内假想敌。
